...
```{r}
adole_data_VS_l <- all_sub_roi_betas[(all_sub_roi_betas$group == 'kid') & (all_sub_roi_betas$roi == 'VS_l-rlearn'), ]
```


beta_mean ~ group * valence * outcome
```{r}
model_adole_VS_l <- lmer(beta_mean ~ task*valence*outcome + 1 + (1|subject_id), 
                          data=adole_data_VS_l)
summary(model_adole_VS_l)
```
This partially replicates Megan's finding. Both analyses find an interaction between valence and outcome. Here, there is a main effect for valence. Megan found a main effect for outcome, and an interaction between task, valence, and outcome. 




```{r}
mdoors_data_C1_r <- all_sub_roi_betas[(all_sub_roi_betas$task == 'mdoors') & (all_sub_roi_betas$roi == 'C1_r-rlearn'), ]
```


beta_mean ~ group * valence * outcome
```{r}
model_mdoors_C1_r <- lmer(beta_mean ~ group*valence*outcome + 1 + (1|subject_id), 
                          data=mdoors_data_C1_r)
summary(model_mdoors_C1_r)
```
This replicates the previous analysis and there are no group differences.


```{r}
ggplot(data = mdoors_data_VS_l, aes(x=outcome, y=beta_mean, fill=valence, alpha=group)) + 
...
```{r}
mdoors_data_VS_l <- all_sub_roi_betas[(all_sub_roi_betas$task == 'mdoors') & (all_sub_roi_betas$roi == 'VS_l-rlearn'), ]
```


beta_mean ~ group * valence * outcome
```{r}
model_mdoors_VS_l <- lmer(beta_mean ~ group*valence*outcome + 1 + (1|subject_id), 
                          data=mdoors_data_VS_l)
summary(model_mdoors_VS_l)
```
