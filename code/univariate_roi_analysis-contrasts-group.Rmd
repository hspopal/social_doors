...
allg_data_pos_mdoors.t_tests = allg_data_pos_mdoors %>%
               group_by(roi) %>%
               summarise(Statistic = funs(t.test(.[group == 'kid'], .[group == 'college'])$statistic),
                         P = t.test(contrast_mean, mu = 0)$p.value,
                         Sig = ifelse(P < 0.05, "*", ""),
                         df = t.test(contrast_mean, mu = 0)$parameter,
                         Estimate = t.test(contrast_mean, mu = 0)$estimate,
...
test <- allg_data_pos_mdoors %>%
        select(-subject_id) %>%
        group_by(roi, group) %>%
        summarize_at(
            vars(-group_cols(), -contrast_mean),
            list(t.test = ~ list(t.test(. ~ contrast_mean))))
...
allg_data_pos_mdoors.t_tests <- allg_data_pos_mdoors %>% 
                                group_by(roi) %>% 
                                do(tidy(t.test(contrast_mean~group, data=.)))
  
# Conduct multiple comparisons correction
stats <- p.adjust(allg_data_pos_mdoors.t_tests$p.value,method='fdr')
P_adjust <- stats
Sig_adjust = ifelse(P_adjust < 0.005, "***", ifelse(P_adjust < 0.01, "**", 
                                             ifelse(P_adjust < 0.05, "*", "")))
allg_data_pos_mdoors.t_tests$P_adjust.
