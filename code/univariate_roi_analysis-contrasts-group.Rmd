...
opts_knit$set(root.dir = "/Volumes/HP-SOC-DOOR/social_doors/derivatives/univariate_roi/")
```
...
library(rstatix)
library(ggpubr)
res.aov <- anova_test(data = allg_data_all_mdoors, dv = contrast_mean, 
                      wid = subject_id, between = group, within = roi)
res.aov
```
...
```{r}
res.aov <- anova_test(data = allg_data_all_social, dv = contrast_mean, 
                      wid = subject_id, between = group, within = roi)
res.aov
```
...
# Positive vs Negative Wins

## Young Adults
```{r}
# Filter data
adult_data <- all_sub_roi_con[all_sub_roi_con$group %in% 'college', ]
adult_data_win <- adult_data[adult_data$contrast %in% 'posVneg_win', ]
adult_data_win <- adult_data_win[adult_data_win$roi %in% hyp_rois, ]

adult_data_win_mdoors <- adult_data_win[adult_data_win$task %in% 'mdoors', ]
adult_data_win_social <- adult_data_win[adult_data_win$task %in% 'social', ]
```
...
```{r}
# Conduct t-tests
adult_data_win_mdoors.t_tests <- mass_t_tests(adult_data_win_mdoors)

y_max <- max(adult_data_win_mdoors.t_tests$CI_u) * 1.1

# Plot
p <- ggbarplot(adult_data_win_mdoors, x = 'roi', 
               y = 'contrast_mean', add = 'mean_se',
               position = position_dodge(0.8))
p + geom_text(aes(label = Sig_adjust, y = y_max), size = 6,
              data = adult_data_win_mdoors.t_tests) + 
  theme(axis.text.x = element_text(angle =90, vjust = 0.5)) + 
  xlab("Regions of Interests") + ylab("Mean Response (positive > negative wins)") + 
  scale_x_discrete(labels=hyp_rois_names)

ggsave('../univariate_roi/adult_posVneg_win_ttest_bar_mdoors.png')
```
...
```{r}
# T-test
adult_data_win_social.t_tests <- mass_t_tests(adult_data_win_social)

y_max <- max(adult_data_win_social.t_tests$CI_u) * 1.1

# Plot
p <- ggbarplot(adult_data_win_social, x = 'roi', 
               y = 'contrast_mean', add = 'mean_se',
               position = position_dodge(0.8))
p + geom_text(aes(label = Sig_adjust, y = y_max), size = 6,
              data = adult_data_win_social.t_tests) + 
  theme(axis.text.x = element_text(angle =90, vjust = 0.5)) + 
  xlab("Regions of Interests") + ylab("Mean Response (positive > negative wins)") + 
  scale_x_discrete(labels=hyp_rois_names)

ggsave('../univariate_roi/adult_posVneg_win_ttest_bar_social.png')
```
...
# Filter data
adole_data <- all_sub_roi_con[all_sub_roi_con$group %in% 'kid', ]
adole_data_win <- adole_data[adole_data$contrast %in% 'posVneg_win', ]
adole_data_win <- adole_data_win[adole_data_win$roi %in% hyp_rois, ]

adole_data_win_mdoors <- adole_data_win[adole_data_win$task %in% 'mdoors', ]
adole_data_win_social <- adole_data_win[adole_data_win$task %in% 'social', ]
...
```{r}
# T-test
adole_data_win_mdoors.t_tests <- mass_t_tests(adole_data_win_mdoors)

y_max <- max(adole_data_win_mdoors.t_tests$CI_u) * 1.1


# Plot
p <- ggbarplot(adole_data_win_mdoors, x = 'roi', 
               y = 'contrast_mean', add = 'mean_se',
               position = position_dodge(0.8))
p + geom_text(aes(label = Sig_adjust, y = y_max), size = 6,
              data = adole_data_win_mdoors.t_tests) + 
  theme(axis.text.x = element_text(angle =90, vjust = 0.5)) + 
  xlab("Regions of Interests") + ylab("Mean Response (positive > negative wins)") + 
  scale_x_discrete(labels=hyp_rois_names)

ggsave('../univariate_roi/adole_posVneg_win_ttest_bar_mdoors.png')
```
...
```{r}
# T-test
adole_data_win_social.t_tests <- mass_t_tests(adole_data_win_social)

y_max <- max(adole_data_win_social.t_tests$CI_u) * 1.1


# Plot
p <- ggbarplot(adole_data_win_social, x = 'roi', 
               y = 'contrast_mean', add = 'mean_se',
               position = position_dodge(0.8))
p + geom_text(aes(label = Sig_adjust, y = y_max), size = 6,
              data = adole_data_win_social.t_tests) + 
  theme(axis.text.x = element_text(angle =90, vjust = 0.5)) + 
  xlab("Regions of Interests") + ylab("Mean Response (positive > negative wins)") + 
  scale_x_discrete(labels=hyp_rois_names)

ggsave('../univariate_roi/adole_posVneg_win_ttest_bar_social.png')
```
...
```{r}
# Filter data
allg_data_win <- all_sub_roi_con[all_sub_roi_con$contrast %in% 'posVneg_win', ]
allg_data_win <- allg_data_win[allg_data_win$roi %in% hyp_rois, ]
```

### Monetary Task

```{r}
allg_data_win_mdoors <- allg_data_win[allg_data_win$task %in% 'mdoors', ]
```

```{r}
res.aov <- anova_test(data = allg_data_win_mdoors, dv = contrast_mean, 
                      wid = subject_id, between = group, within = roi)
res.aov
```
...
```{r}
# ANOVA
model_allg_win_mdoors <- aov(contrast_mean ~ group*roi,
                           data = allg_data_win_mdoors)
report(model_allg_win_mdoors)
```


```{r}
allg_data_win_mdoors.t_tests <- mass_twos_t_tests(allg_data_win_mdoors)

allg_data_win_mdoors <- bar_fill_one_sample(allg_data_win_mdoors, 
                                            adole_data_win_mdoors.t_tests,
                                            adult_data_win_mdoors.t_tests)

fill.colors <- c(adole_not = "#FFFFFF", adole_sig = "#7fb8d8", adult_not ="#FFFFFF", adult_sig = "#eaae7f")
outline.colors <- c("#D55E00", "#0072B2")

y_max <- max(allg_data_win_mdoors.t_tests$CI_u) * 1.1


# Plot
p <- ggbarplot(allg_data_win_mdoors, x = 'roi', 
               y = 'contrast_mean', add = 'mean_se', add.params = list(group = "group"),
               group = 'group', color='group', position = position_dodge(0.8), 
               palette = outline.colors, fill = 'plot_color')
p + geom_text(aes(label = Sig_adjust, y = y_max), size = 6,
              data = allg_data_win_mdoors.t_tests) + 
  theme(axis.text.x = element_text(angle =90, vjust = 0.5), legend.position='right') + 
  xlab("Regions of Interests") + ylab("Mean Response (positive > negative wins)") + 
  scale_x_discrete(labels=hyp_rois_names) + 
  scale_fill_manual(values=fill.colors) + 
...
ggsave('../univariate_roi/allp_posVneg_win_anova_bar_mdoors.png')
```
...
```{r}
# Filter data
allg_data_win_social <- allg_data_win[allg_data_win$task %in% 'social', ]
```

```{r}
# ANOVA
model_allg_win_social <- aov(contrast_mean ~ group*roi,
                           data = allg_data_win_social)
report(model_allg_win_social)
```


```{r}
res.aov <- anova_test(data = allg_data_win_social, dv = contrast_mean, 
                      wid = subject_id, between = group, within = roi)
res.aov
```
...
```{r}
allg_data_win_social.t_tests <- mass_twos_t_tests(allg_data_win_social)

allg_data_win_social <- bar_fill_one_sample(allg_data_win_social, 
                                            adole_data_win_social.t_tests,
                                            adult_data_win_social.t_tests)

fill.colors <- c(adole_not = "#FFFFFF", adole_sig = "#7fb8d8", adult_not ="#FFFFFF", adult_sig = "#eaae7f")
outline.colors <- c("#D55E00", "#0072B2")

y_max <- max(allg_data_win_social.t_tests$CI_u)


# Plot
p <- ggbarplot(allg_data_win_social, x = 'roi', 
               y = 'contrast_mean', add = 'mean_se', add.params = list(group = "group"),
               group = 'group', color='group', position = position_dodge(0.8), 
               palette = outline.colors, fill = 'plot_color')
p + geom_text(aes(label = Sig_adjust, y = y_max), size = 6,
              data = allg_data_win_social.t_tests) + 
  theme(axis.text.x = element_text(angle =90, vjust = 0.5), legend.position='right') + 
  xlab("Regions of Interests") + ylab("Mean Response (positive > negative wins)") + 
  scale_x_discrete(labels=hyp_rois_names) + 
  scale_fill_manual(values=fill.colors) + 
...
ggsave('../univariate_roi/allp_posVneg_win_anova_bar_social.png')
```
...
p1 <- ggbarplot(allg_data_pos_mdoors, x = 'roi', 
               y = 'contrast_mean', add = 'mean_se', add.params = list(group = "group"),
               group = 'group', color='group', position = position_dodge(0.8), 
               palette = outline.colors, fill = 'plot_color', ylim = c(-0.4,0.6)) + 
...
p2 <- ggbarplot(allg_data_win_mdoors, x = 'roi', 
               y = 'contrast_mean', add = 'mean_se', add.params = list(group = "group"),
               group = 'group', color='group', position = position_dodge(0.8), 
               palette = outline.colors, fill = 'plot_color', ylim = c(-0.4,0.6)) +
  geom_text(aes(label = Sig_adjust, y = 0.3), size = 6,
  ...
  ggtitle('B. Positive Wins > Negative Wins Contrast') + 
  scale_x_discrete(labels=hyp_rois_names) + 
  scale_fill_manual(values=fill.colors) + 
  guides(fill = FALSE) + 
  scale_colour_manual(name = 'Group', values = outline.colors, 
...
p2 <- ggbarplot(allg_data_win_social, x = 'roi', 
               y = 'contrast_mean', add = 'mean_se', add.params = list(group = "group"),
               group = 'group', color='group', position = position_dodge(0.8), 
               palette = outline.colors, fill = 'plot_color', ylim = c(-0.4,0.6)) +
  geom_text(aes(label = Sig_adjust, y = 0.3), size = 6,
