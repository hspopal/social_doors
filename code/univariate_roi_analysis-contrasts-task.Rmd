...
ggsave('../univariate_roi/allg_pos_winVlos_ttest_bar_mdoors.png')
```
...
ggsave('../univariate_roi/allg_pos_winVlos_ttest_bar_social.png')
```
...
ggsave('../univariate_roi/allg_all_winVlos_ttest_bar_mdoors.png')
```
...
ggsave('../univariate_roi/allg_all_winVlos_ttest_bar_social.png')
```
...
```{r}
# ANOVA
model_allg_all <- aov(contrast_mean ~ task*roi,
                           data = allg_data_all)
report(model_allg_all)
```
...
ggsave('../univariate_roi/allg_all_winVlos_anova_bar.png')
```
...
# Plot
p1 <- ggbarplot(allg_data_pos, x = 'roi', 
               y = 'contrast_mean', add = 'mean_se', add.params = list(group = "task"),
               group = 'task', color='task', position = position_dodge(0.8), 
               palette = outline.colors, fill = 'plot_color') + 
...
p2 <- ggbarplot(allg_data_all, x = 'roi', 
               y = 'contrast_mean', add = 'mean_se', add.params = list(group = "task"),
               group = 'task', color='task', position = position_dodge(0.8), 
               palette = outline.colors, fill = 'plot_color') +
  geom_text(aes(label = Sig_adjust, y = 0.3), size = 6,
...
annotate_figure(multi_plot, top = text_grob("ROI Responses for Reward Tasks\n", 
               face = "bold", size = 18))


ggsave('../univariate_roi/allp_anova_bar.png')
```
...
```{r}
allg_data_pos_mdoors.t_tests$task <- 'mdoors'
allg_data_pos_mdoors.t_tests$contrast <- 'positive'

allg_data_pos_social.t_tests$task <- 'social'
allg_data_pos_social.t_tests$contrast <- 'positive'

allg_data_all_mdoors.t_tests$task <- 'mdoors'
allg_data_all_mdoors.t_tests$contrast <- 'all'

allg_data_all_social.t_tests$task <- 'social'
allg_data_all_social.t_tests$contrast <- 'all'


combined_ttests <- rbind(allg_data_pos_mdoors.t_tests,
                         allg_data_pos_social.t_tests,
                         allg_data_all_mdoors.t_tests,
                         allg_data_all_social.t_tests)
...
```{r}
# Sort dataframe
one_sample_table <- combined_ttests[order(combined_ttests$contrast,
...
# Reorder columns
one_sample_table <- one_sample_table[, c('contrast', 'task', "roi", "Estimate", 
                                         "df", "Statistic",
                                           "P_adjust", "CI")]

colnames(one_sample_table) <- c('Contrast', 'Task', 'ROI','Mean','df','t-statistic','p','CI')

```

```{r}
one_sample_table %>%
  kbl(caption = "Univariate ROI Results") %>%
  kable_classic(full_width = F, html_font = "Times New Roman") %>%
  save_kable("mass_one_sample_ttests-task.html")
